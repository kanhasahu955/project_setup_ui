...
export function AuthLayout({ children, rightContent, className = "" }: AuthLayoutProps) {
  return (
    <div
      className={`min-h-screen min-h-[100dvh] flex flex-col md:flex-row md:gap-0 bg-slate-100 p-5 sm:p-6 md:p-8 overflow-y-auto overflow-x-hidden md:overflow-hidden md:h-screen md:min-h-[100dvh] ${className}`.trim()}
    >
      {/* Single bordered container: on mobile grows with content so outer scrolls; on desktop fixed */}
      <div
        className={`
          w-full flex-none md:flex-1 flex flex-col md:flex-row min-h-0 min-w-0
          rounded-xl md:rounded-2xl overflow-hidden
          border border-slate-200 bg-white shadow-sm
        `.trim().replace(/\s+/g, " ")}
      >
        {/* Left: form panel — on mobile no overflow clip so content can grow */}
        <main
          className={`
            w-full max-w-full md:w-[48%] lg:w-[44%] xl:max-w-[520px]
            flex flex-col justify-center md:justify-center items-center min-h-0
            overflow-visible md:overflow-hidden bg-white shrink-0
